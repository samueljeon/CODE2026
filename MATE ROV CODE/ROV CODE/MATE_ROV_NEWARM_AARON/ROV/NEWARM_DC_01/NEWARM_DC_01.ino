...
float readDistanceCM() {
  while (Serial3.available()) {
    Serial3.read(); // clear old bytes
  }

  Serial3.write(0x55);  // trigger measurement

  unsigned long startTime = millis();

  while (Serial3.available() < 4) {
    if (millis() - startTime > 100) {
      return -1.0f; // timeout / no reading
    }
  }
  ...
  uint8_t b3 = Serial3.read();

  if (b0 != 0xFF) {
    return -2.0f; // bad start byte
  }

  uint8_t checksum = (b0 + b1 + b2) & 0xFF;

  if (checksum != b3) {
    return -3.0f; // bad checksum
  }

  int distanceMM = (b1 << 8) | b2;
...
void sendTelemetry() {
  ...
  yaw_deg += gyroEvent.gyro.z * 180.0f / PI * dt;
