...
static float ApplyExpo(float input, float expo = 0.7f)
{
    return expo * input * input * input
         + (1.0f - expo) * input;
}
...
int main() {
    ...
    while (!quit) {
        ...
        tright = ApplyExpo(std::clamp(tright, -1.0f, 1.0f), 0.7f);
        ...
        ImGui::SeparatorText("MPU6050");

        ImGui::Text(
            "Pitch: %.2f deg   Roll: %.2f deg   Temp: %.2f C",
            incoming.pitch_deg,
            incoming.roll_deg,
            incoming.temp
        );
        ...
        ImGui::Text(
            "Accel X: %.3f g   Accel Y: %.3f g   Accel Z: %.3f g",
            incoming.accel_x_g,
            incoming.accel_y_g,
            incoming.accel_z_g
        );
        ...
        float totalG =
            sqrtf(incoming.accel_x_g * incoming.accel_x_g +
                incoming.accel_y_g * incoming.accel_y_g +
                incoming.accel_z_g * incoming.accel_z_g);

        ImGui::Text("Total G: %.3f g", totalG);
